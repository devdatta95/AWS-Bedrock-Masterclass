...
# ------------------------------------------------------------------------
# The `bedrock-runtime` service is used to invoke text / chat / image
# inference models such as Titan, Claude, Llama etc.
# Ensure that:
# 1. Your AWS credentials are configured.
# 2. The chosen region supports the model you want to call.
client = boto3.client(
    service_name='bedrock-runtime',
    region_name="us-east-1"
)

# Model ID of Amazon Titan Text Express
titan_model_id = 'amazon.titan-text-express-v1'


def generate_summary(prompt: str) -> str:
    """
    Generate a short summary using Amazon Titan Text Express.

    Parameters
    ----------
    prompt : str
        The text input given to the model, containing instructions and
        the conversation transcript to summarize.

    Returns
    -------
    str
        The summarized output text returned by the Titan model.

    Notes
    -----
    - Uses `invoke_model` API from Bedrock Runtime.
    - Temperature = 0 ensures deterministic output.
    - Titan models require input formatted as JSON with fields:
        - inputText
        - textGenerationConfig
    """
    # Prepare model input configuration as JSON
    titan_config = json.dumps({
        "inputText": prompt,
        "textGenerationConfig": {
            "maxTokenCount": 4096,   # Max tokens Titan should generate
            "stopSequences": [],     # No forced stopping sequences
            "temperature": 0,        # Deterministic output
            "topP": 1                # Sampling configuration (kept simple)
        }
    })

    # --------------------------------------------------------------------
    # Invoke Titan through the Bedrock Runtime client
    # --------------------------------------------------------------------
    ...
    # The response body is streamed, so we read and decode it
    response_body = json.loads(response.get('body').read())

    # Titan returns a list called "results"; extract the generated text
    return response_body.get('results')[0].get('outputText')


# ------------------------------------------------------------------------
# Example usage: Summarizing a phone conversation
# ------------------------------------------------------------------------
prompt = """
